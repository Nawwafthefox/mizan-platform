...
import com.mizan.service.V3ExcelImportService.ImportResult;
        ...
        Thread worker = new Thread(() -> {
            ...
            try {
                for (int i = 0; i < bytesList.size(); i++) {
                    ImportResult result = importSvc.importByType(
                        type, bytesList.get(i), names.get(i), tenantId, importId);
                    totalSaved  += result.autoSaved();
                    totalStaged += result.stagedForReview();
                    log.info("V3 {} '{}' → {} auto-saved, {} staged for review",
                        type, names.get(i), result.autoSaved(), result.stagedForReview());
                }
                statusSvc.complete(importId, totalSaved, totalStaged);
            } catch (Exception e) {
